...
import type {
  DiceRollResult, DieColor, GamePhase, SustainedX,
} from '../../core/types';
...
export function MobileResultsView({
  ...
  // Don't auto-open the sheet — the action strip shows counts.
  // User taps the chip to see detailed results with DEL/R/S/L.
  useEffect(() => {
    if (gamePhase === 'ROLLING') setSheetOpen(false);
  }, [gamePhase]);
  ...
  return (
    <>
      ...
      {/* Toggle chip — bottom center */}
      {!sheetOpen && (
        <div style={s.chipWrap}>
          <button style={s.toggleChip} onClick={() => setSheetOpen(true)}>
            {busy ? spinningLabel(gamePhase) : `≡  ${totalActive} dados`}
          </button>
        </div>
      )}
...
const s: Record<string, React.CSSProperties> = {
  ...
  actionCnt: {
    ...
    minWidth: 14, fontFamily: FONT_FAMILY,
  },
  chipWrap: {
  ...
  toggleChip: {
    ...
    border: '1px solid #2a4a6a', borderRadius: 20,
    color: '#6a9aaa', padding: '6px 20px', fontSize: 11,
  ...
  mobileSheet: {
    ...
    fontFamily: FONT_FAMILY, zIndex: 40,
  ...
  sheetClose: {
    marginLeft: 'auto', fontFamily: FONT_FAMILY,
  ...
  animStatus: {
    ...
    letterSpacing: 1, flex: 1, textAlign: 'center',
  },
  totalLabel: { fontSize: 10, letterSpacing: 1, fontFamily: FONT_FAMILY },
};
